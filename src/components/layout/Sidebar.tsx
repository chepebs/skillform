...
const navItems: NavItem[] = [{
  ...
  icon: Home,
  path: '/dashboard',
  roles: ['master_admin', 'organizer_admin', 'department_director']
}, {
  labelKey: 'common.navigation.myProfile',
  icon: User,
  path: '/profile/me'
}, {
  labelKey: 'common.navigation.directory',
  icon: Users,
  path: '/directory'
}, {
  labelKey: 'common.navigation.groups',
  icon: Folder,
  path: '/admin/organizer/groups',
  roles: ['organizer_admin', 'master_admin']
}, {
  labelKey: 'common.navigation.myTeam',
  icon: Users,
  path: '/admin/director/team',
  roles: ['department_director', 'master_admin']
}, {
  labelKey: 'common.navigation.departmentInfo',
  icon: Building,
  path: '/admin/director/info',
  roles: ['department_director', 'master_admin']
}, {
  labelKey: 'common.navigation.userManagement',
  icon: Shield,
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const logoFilter = resolvedTheme === 'dark' ? 'brightness(0) invert(1)' : 'brightness(0)';
  return <aside className={cn('fixed left-0 top-0 z-40 h-screen bg-sidebar border-r border-sidebar-border transition-all duration-300 ease-smooth', collapsed ? 'w-14' : 'w-56')}>
      <div className="flex flex-col h-full">
        ...
        <nav className="flex-1 overflow-y-auto py-3 px-2">
          <ul className="space-y-1">
            ...
            return <li key={item.path}>
                  <NavLink to={item.path} className={cn('flex items-center gap-2 px-2 py-2.5 transition-all duration-200 text-sm', active ? 'bg-foreground text-background font-semibold' : 'text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground')} title={collapsed ? label : undefined}>
        ...
        {/* User Info */}
        {!collapsed && <div className="p-4 border-t border-sidebar-border">
        ...
        {/* Toggle Button */}
        <button onClick={onToggle} className="absolute -right-2.5 top-20 p-1 bg-card border border-border hover:border-primary transition-colors">
